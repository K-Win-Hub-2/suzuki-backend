import { CustomerData, UserDatas, customerDataToImplementDatabase, superAdminAccountDataToImplementDatabase } from "../helpers/accountDataMappers";
...
class AdminAccountService{
    ...
    public async updateById(id: mongoose.Types.ObjectId,datas: UserDatas){
        ...
        //  }
         ...
         const result = await AdminUsers.findOneAndUpdate({_id: id}, formattedData, { new: true })
         return successResponse({ statusCode:200, message: "Admin Account Updated successfully", data: result})
    }
    public async delete(id: mongoose.Types.ObjectId){
            const result = await AdminUsers.findByIdAndUpdate(id, {isDeleted: true})
            return successResponse({ statusCode:200, message: "Account deleted successfully", data: result})
    }
...
class CustomerAccountService {
    ...
    public async create(data: CustomerData){
         ...
         return successResponse({ statusCode:200, message: "User Account Created successfully", data: result})
    }
    public async readById(id: mongoose.Types.ObjectId){
...
export { UserAccountServiceFactory, AdminAccountService, CustomerAccountService }